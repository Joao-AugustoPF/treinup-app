...
const CLASS_STATUS_DISPLAY: Record<
  Class['status'],
  { label: string; color: string }
> = {
  scheduled: { label: 'Agendada', color: '#2196F3' },
  confirmed: { label: 'Confirmada', color: '#4CAF50' },
  in_progress: { label: 'Em Andamento', color: '#FF9800' },
  completed: { label: 'Concluída', color: '#9E9E9E' },
  cancelled: { label: 'Cancelada', color: '#F44336' },
  no_show: { label: 'Não Compareceu', color: '#F44336' },
};

const isCheckInAllowed = (status: Class['status']): boolean => {
...
export default function ClassesScreen() {
  ...
  const [classTypes, setClassTypes] = useState<ClassType[]>(['funcional']);
                      ...
                      <View style={styles.classTypeContainer}>
                        ...
                        <View
                          style={[
                            ...
                            {
                              backgroundColor:
                                CLASS_STATUS_DISPLAY[fitnessClass.status]
                                  .color + '20',
                            },
                          ]}
                        >
                          ...
                          >
                            {CLASS_STATUS_DISPLAY[fitnessClass.status].label}
                          </Text>
                        </View>
                      </View>
                  ...
                  {fitnessClass.isCheckedIn ? (
                    ...
                    <TouchableOpacity
                      style={[
                        ...
                        {
                          backgroundColor: isCheckInAllowed(fitnessClass.status)
                            ? paperTheme.colors.primary
                            : paperTheme.colors.surfaceVariant,
                          opacity: isCheckInAllowed(fitnessClass.status)
                            ? 1
                            : 0.5,
                        },
                      ]}
                      onPress={() => handleCheckIn(fitnessClass.id)}
                      disabled={!isCheckInAllowed(fitnessClass.status)}
                    >
                      <CheckCircle2
                        size={20}
                        color={
                          isCheckInAllowed(fitnessClass.status)
                      ...
                      <Text
                        style={[
                          ...
                          {
                            color: isCheckInAllowed(fitnessClass.status)
                              ? paperTheme.colors.onPrimary
                              : paperTheme.colors.onSurfaceVariant,
                          },
                        ]}
