...
export class PaymentService {
  ...
  static async createPaymentIntent(
    ...
    const paymentMethod = MOCK_PAYMENT_METHODS.find(
  ...
  static async confirmPayment(
    ...
    const paymentIntent = MOCK_PAYMENT_INTENTS.find(
      (pi) => pi.id === paymentIntentId
    );
    if (!paymentIntent) {
  ...
  static async deletePaymentMethod(
    ...
    await new Promise((resolve) => setTimeout(resolve, 500));
    const index = MOCK_PAYMENT_METHODS.findIndex(
