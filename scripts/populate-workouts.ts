...
async function createWorkoutSets(workoutId: string, exercises: any[]) {
  console.log(`Criando sets para o workout ${workoutId}`);
  console.log(`Exercícios disponíveis: ${exercises.length}`);
  ...
  console.log(
    `Selecionados ${selectedExercises.length} exercícios para este workout`
  );

  for (let i = 0; i < selectedExercises.length; i++) {
    const exercise = selectedExercises[i];
    const numSets = faker.number.int({ min: 3, max: 5 });
    console.log(`Criando ${numSets} sets para o exercício ${exercise.name}`);
    ...
    for (let j = 0; j < numSets; j++) {
      try {
        ...
        console.log('Criando set com dados:', setData);

        const createdSet = await databases.createDocument(
          DATABASE_ID,
        ...
        console.log(`Set criado com sucesso: ${createdSet.$id}`);
      } catch (error) {
        console.error('Erro ao criar set:', error);
        console.error('Dados do set que falhou:', {
          workoutId,
          exerciseId: exercise.$id,
          order: i + 1,
          series: numSets,
...
async function populateWorkouts() {
  try {
    ...
    for (const plan of WORKOUT_PLANS) {
      ...
      if (workout) {
        console.log(`Workout criado com sucesso: ${workout.$id}`);
        console.log(`Criando sets para o workout: ${workout.$id}`);
        await createWorkoutSets(workout.$id, exercises);
      } else {
        console.error('Falha ao criar workout para o plano:', plan);
      }
    }
