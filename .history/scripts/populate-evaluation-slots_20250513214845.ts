...
async function generateSlotsForNextDays(numberOfDays: number, trainerIds: string[]) {
    ...
    for (const trainerId of trainerIds) {
      ...
      for (const timeSlot of availableSlots) {
        ...
        try {
          ...
          console.log(`Criado slot: ${slotStart.toLocaleTimeString()} - Treinador: ${trainerId}`);
        } catch (error) {
...
async function populateEvaluationSlots() {
  try {
    ...
    if (trainersResponse.documents.length === 0) {
      console.log('Nenhum treinador encontrado. Criando um treinador de exemplo...');
