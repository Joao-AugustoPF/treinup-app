...
export const secureStore = {
  async setSessionId(sessionId: string) {
    ...
    try {
      await SecureStore.setItem(KEYS.SESSION_ID, sessionId);
  ...
  async getSessionId() {
    ...
    try {
      const result = await SecureStore.getItem(KEYS.SESSION_ID);
  ...
  async setJWT(jwt: string) {
    ...
    try {
      await SecureStore.setItem(KEYS.JWT, jwt);
  ...
  async getJWT() {
    ...
    try {
      const result = await SecureStore.getItem(KEYS.JWT);
  ...
  async setActiveTenantId(tenantId: string) {
    ...
    try {
      await SecureStore.setItem(KEYS.ACTIVE_TENANT_ID, tenantId);
  ...
  async getActiveTenantId() {
    ...
    try {
      const result = await SecureStore.getItem(KEYS.ACTIVE_TENANT_ID);
