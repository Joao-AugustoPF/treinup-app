...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const removePhoto = async () => {
    try {
      setIsUploading(true);
      // Use a default avatar URL instead of empty string
      const defaultAvatarUrl =
        'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=1000&auto=format&fit=crop';
      await ProfileService.updateUserAvatar(user, defaultAvatarUrl);
      onUploadComplete();
    } catch (error) {
  ...
  const showUploadOptions = () => {
    ...
    Alert.alert('Upload Photo', 'Choose an option', options, {
      cancelable: true,
    });
  };
