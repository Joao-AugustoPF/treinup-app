import { client, USER_AVATARS_BUCKET_ID } from '@/src/api/appwrite-client';
...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const uploadImage = async (asset: any) => {
    try {
      ...
      // 1️⃣ wrap the local file URI
      const input = InputFile.fromFile(
        asset.uri,
        asset.fileName ?? `avatar-${user?.$id}.${new Date().getTime()}`
      );
