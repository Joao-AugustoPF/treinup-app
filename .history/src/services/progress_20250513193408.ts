...
export class ProgressService {
  ...
  static async getLatestMetrics(
    ...
    // 1) Busca **todas** as métricas do usuário (ou com um limit grande)
    const profile = await this.getUserProfile(user.$id);
    const profileId = profile.$id;
    const all = await db.listDocuments(DATABASE_ID, METRICS_COLLECTION_ID, [
      Query.equal('memberProfileId', profileId),
      Query.equal('tenantId', DEFAULT_TENANT_ID),
      Query.orderAsc('recordedAt'),
      Query.limit(1000), // garante que traga tudo
    ]);

    // 2) Para cada tipo, guarda o registro mais recente
    const latestByType: Record<string, { date: Date; value: number }> = {};

    all.documents.forEach((doc) => {
      const type = doc.type as MetricType;
      const date = new Date(doc.recordedAt);
      const value = doc.value as number;

      if (
        !latestByType[type] ||
        date.getTime() > latestByType[type].date.getTime()
      ) {
        latestByType[type] = { date, value };
      }
    });

    // 3) Converte para o formato esperado
    return (Object.keys(latestByType) as MetricType[]).reduce((acc, type) => {
      acc[type] = latestByType[type].value;
      return acc;
