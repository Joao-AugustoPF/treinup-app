    ...
    try {
      ...
      const posts = await Promise.all(
        response.documents.map(async (doc) => {
          const authorProfile = await client.databases.getDocument(
            DATABASE_ID,
          ...
          return {
            ...
            author: {
              ...
              image: authorProfile.avatarUrl || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(authorProfile.name),
    ...
    try {
      ...
      return {
        ...
        author: {
          id: profile.$id,
          name: profile.name,
          image: profile.avatarUrl || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(profile.name),
          role: profile.role
        },
        likes: 0,
        comments: 0,
