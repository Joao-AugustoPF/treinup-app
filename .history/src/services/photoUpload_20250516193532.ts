...
import { InputFile } from 'appwrite/storage';
...
export class PhotoUploadService {
  ...
  static async uploadProfilePhoto(
    ...
    try {
      ...
      // Step 2: Upload to Appwrite using InputFile.fromPath
      const uploadResult = await storage.createFile(
        USER_AVATARS_BUCKET_ID,
        fileId,
        InputFile.fromPath(imageAsset.uri, fileName),
        ['read("any")']
      );

      console.log('Upload successful:', uploadResult);

      // Step 3: Get the file view URL
      ...
      // Step 4: Update user profile with new photo URL
