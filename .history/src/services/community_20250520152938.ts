    ...
    try {
      ...
      const posts = await Promise.all(
        response.documents.map(async (doc: any): Promise<Post | null> => {
          ...
          try {
            // Validate the authorProfileId format
            if (!/^[a-zA-Z0-9]{1,36}$/.test(doc.authorProfileId)) {
              console.error('Invalid authorProfileId format:', {
                postId: doc.$id,
                authorProfileId: doc.authorProfileId,
                length: doc.authorProfileId.length,
                containsInvalidChars: /[^a-zA-Z0-9]/.test(doc.authorProfileId),
                startsWithUnderscore: doc.authorProfileId.startsWith('_'),
              });
              return null;
            }

            console.log('Fetching author profile for post:', {
              postId: doc.$id,
              authorProfileId: doc.authorProfileId,
            });
