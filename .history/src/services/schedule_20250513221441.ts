...
export class ScheduleService {
  ...
  static async getAvailableSlots(
    ...
    try {
      ...
      console.log(`Found ${slotsResponse.documents.length} total slots for the selected date`);
      ...
      console.log(`Found ${bookingsResponse.documents.length} booked slots in total`);

      // Extract booked slot IDs, handling both string IDs and object references
      const bookedSlotIds = bookingsResponse.documents.map((booking) => {
        // If evaluationSlots is a string ID
        if (typeof booking.evaluationSlots === 'string') {
          return booking.evaluationSlots;
        }
        // If evaluationSlots is an object with $id
        else if (booking.evaluationSlots && booking.evaluationSlots.$id) {
          return booking.evaluationSlots.$id;
        }
        return null;
  ...
  static async rescheduleAppointment(
    ...
    try {
      ...
      // Find the user's profile ID to verify ownership
      const profilesResponse = await db.listDocuments(
        DATABASE_ID,
        PROFILES_COLLECTION_ID,
        [Query.equal('userId', user.$id)]
      ...
      // Verify booking ownership - handling both object and string IDs
      const bookingMemberProfileId =
        typeof currentBooking.memberProfileId === 'object'
          ? currentBooking.memberProfileId.$id
          : currentBooking.memberProfileId;

      if (bookingMemberProfileId !== userProfileId) {
        throw new Error(
      ...
      const bookingsResponse = await db.listDocuments(
        ...
        [
          ...
          Query.equal('status', 'booked'),
          Query.notEqual('$id', scheduleId), // Exclude the current booking being updated
        ]
      );
