...
// ID da academia padrão do sistema
export const DEFAULT_GYM_ID = '6822c139dcce804a0582'; // ID da academia no sistema
...
export class ProgressService {
  ...
  static async getUserMetrics(
    ...
    // Em uma aplicação real, faríamos uma consulta ao Appwrite:
    // 1. Obter o profile do usuário pelo user.id
    // 2. Consultar métricas onde memberProfileId = profile.$id e tenantId = DEFAULT_TENANT_ID
  ...
  static async addMetric(
    ...
    // Em uma aplicação real com Appwrite:
    // 1. Obter o profile do usuário pelo user.id
    // 2. Criar um documento na coleção Metrics com:
    //    - memberProfileId = profile.$id
    //    - tenantId = DEFAULT_TENANT_ID
    //    - type, value e outros campos

    const newMetric: Metric = {
      id: Math.random().toString(36).substring(7),
      tenantId: DEFAULT_TENANT_ID, // Usar o tenant padrão (time)
      recordedAt: new Date(),
      ...metric,
    };
  ...
  static async addMetricBatch(
    ...
    // Em uma aplicação real com Appwrite:
    // Seria semelhante ao addMetric, mas criando múltiplos documentos
