...
import { useRealtime } from '@appwrite/react';
...
export default function ClassesScreen() {
  ...
  const { subscribe: subscribeClasses } = useRealtime();
  const { subscribe: subscribeBookings } = useRealtime();
  ...
  useEffect(() => {
    if (currentGym) {
      ...
      try {
        ...
        unsubscribeClasses = subscribeClasses(
          `databases.${DATABASE_ID}.collections.${CLASSES_COLLECTION_ID}.documents`,
          (response) => {
            console.log('Classes realtime event:', response.events);
            ...
            if (
              ...
              console.log('Classes changed, refreshing...');
              fetchClasses();
            }
          }
        );
        ...
        unsubscribeBookings = subscribeBookings(
          ...
          (response) => {
            console.log('Bookings realtime event:', response.events);
            if (
              response.events.includes(
                'databases.*.collections.*.documents.*.create'
              ) ||
              ...
              console.log('Bookings changed, refreshing classes...');
              fetchClasses();
            }
          }
        );
      ...
      return () => {
        try {
          if (unsubscribeClasses) {
            console.log('Cleaning up classes subscription...');
            unsubscribeClasses();
          }
  ...
  }, [currentGym, subscribeClasses, subscribeBookings]);
