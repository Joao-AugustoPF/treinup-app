...
export default function ScheduleScreen() {
  ...
  useEffect(() => {
    const loadTimeSlots = async () => {
      if (selectedDate) {
        try {
          setSlotLoading(true);
          const slots = await ScheduleService.getAvailableSlots(selectedDate, selectedTrainer);
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      Alert.alert('Erro', 'Falha ao agendar avaliação. Verifique se o horário ainda está disponível.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Agende e gerencie suas avaliações físicas</Text>
          <TouchableOpacity
        ...
        <ScrollView style={styles.content}>
          {schedules.length === 0 ? (
            ...
            <View>
              <Text style={styles.sectionLabel}>Próximas Avaliações</Text>
              {schedules
              ...
              {schedules
                .filter(s => s.status === 'cancelled' || new Date(s.date) < new Date())
                .map((schedule) => (
                  <ScheduleCard 
                    key={schedule.id} 
                    schedule={schedule} 
  ...
  return (
    <View style={styles.container}>
      <View style={styles.bookingHeader}>
        ...
        <Text style={styles.bookingTitle}>Agendar Avaliação</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.stepContainer}>
          <View style={styles.stepIndicator}>
            <View style={[styles.stepNumber, selectedDate ? styles.stepCompleted : {}]}>
          ...
          <View style={styles.stepIndicator}>
            <View style={[styles.stepNumber, selectedTrainer ? styles.stepCompleted : {}]}>
          ...
          <View style={styles.stepIndicator}>
            <View style={[styles.stepNumber, selectedTime ? styles.stepCompleted : {}]}>
        ...
        <View style={styles.section}>
          ...
          {dateLoading ? (
            <ActivityIndicator size="small" color="#00E6C3" style={styles.sectionLoader} />
          ) : (
            <ScrollView
              horizontal
        ...
        <View style={styles.section}>
          ...
          >
            {availableTrainers.length === 0 ? (
              <View style={styles.noDataContainer}>
                <Text style={styles.noDataText}>Nenhum treinador disponível</Text>
              </View>
            ) : (
          ...
          {selectedDate && selectedTrainer && selectedTime && (
            <View style={styles.summaryContainer}>
              ...
              <View style={styles.summaryRow}>
                ...
                <Text style={styles.summaryText}>
                  {availableTrainers.find(t => t.id === selectedTrainer)?.name || 'Treinador selecionado'}
                </Text>
              </View>
...
function ScheduleCard({ 
  ...
  const trainerName = trainers.find(t => t.id === schedule.trainerId)?.name || 'Treinador';
