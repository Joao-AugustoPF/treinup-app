      ...
      <ScrollView>
        ...
        <View style={styles.exerciseDetails}>
          ...
          <View style={styles.seriesContainer}>
            {Array.from({ length: Math.min(exercise.sets, 5) }).map((_, i) => (
              <View key={i} style={styles.seriesIconWrapper}>
                <Repeat size={24} color="#00E6C3" />
                <Text style={styles.seriesRepsText}>{exercise.reps}</Text>
              </View>
            ))}
            {exercise.sets > 5 && (
              <Text style={[styles.exerciseMuscle, { marginLeft: 5, alignSelf: "center" }]}>+{exercise.sets - 5}</Text>
            )}
          </View>

          {/* descanso em linha separada */}
          <Text style={styles.restText}>Descanso: {exercise.rest}</Text>
        </View>
