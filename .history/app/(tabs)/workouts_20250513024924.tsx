...
// import { useGym } from '@/contexts/gym';
...
// Define o tipo estendido para o exercício que inclui todas as propriedades necessárias para a UI
...
export default function WorkoutsScreen() {
  ...
  const loadWorkouts = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      console.log('Loading workouts for gym:', currentGym.name, 'with tenantId:', currentGym.tenantId || currentGym.id);
      const userWorkouts = await WorkoutService.getUserWorkouts(
        user,
        currentGym.tenantId || currentGym.id
      );
  ...
  // Função para transformar os dados do workout para o formato que a UI espera
  const formatWorkoutForUI = (workout: Workout) => {
    return {
      id: workout.id,
      name: workout.title || 'Treino sem título',
      focus: workout.plan || 'Plano geral',
      image: 'https://via.placeholder.com/500x300/333/fff?text=Workout',
      exercises: workout.sets?.map(set => {
        const exerciseInfo = set.exercise || {
          id: set.exerciseId || set.id,
          name: `Exercício ${set.order}`,
          muscleGroup: 'Grupo muscular não definido'
        };
          ...
          <View style={styles.workoutsContainer}>
            {loading ? (
              ...
              workouts.map((workout) => {
                const uiWorkout = formatWorkoutForUI(workout);
                return (
                <View key={uiWorkout.id} style={styles.workoutCard}>
                  <Image
                    source={{ uri: uiWorkout.image }}
                    style={styles.workoutImage}
                  />
                  <View style={styles.workoutOverlay} />
                  <View style={styles.workoutContent}>
                    <View>
                      <Text style={styles.workoutName}>{uiWorkout.name}</Text>
                      <Text style={styles.workoutFocus}>{uiWorkout.focus}</Text>
                    </View>
                    <View style={styles.exerciseCount}>
                      <Text style={styles.exerciseCountText}>
                        {uiWorkout.exercises.length} exercícios
                      </Text>
                    </View>
                  </View>

                  <View style={styles.exerciseList}>
                    {uiWorkout.exercises.map((exercise) => (
                      <TouchableOpacity
                        key={exercise.id}
