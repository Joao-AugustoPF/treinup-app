        ...
        <ScrollView style={styles.content}>
          {schedules.length === 0 ? (
            ...
            <View>
              ...
              {/* Avaliações completadas (data passada e status booked ou attended) */}
              {schedules.filter(
                (s) =>
                  (s.status === 'booked' || s.status === 'attended') &&
                  new Date(s.date) < new Date()
              ).length > 0 && (
                <>
                  <Text style={styles.sectionLabel}>Avaliações Realizadas</Text>
                  {schedules
                    .filter(
                      (s) =>
                        (s.status === 'booked' || s.status === 'attended') &&
              ...
              {schedules.filter((s) => s.status === 'cancelled').length > 0 && (
                <>
                  <Text style={styles.sectionLabel}>Avaliações Canceladas</Text>
                  {schedules
