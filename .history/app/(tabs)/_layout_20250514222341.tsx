...
export default function TabLayout() {
  ...
  useEffect(() => {
    // Load the notification icon preference
    const loadNotificationIconPreference = async () => {
      if (user) {
        try {
          const profile = await ProfileService.getUserProfile(user);
          setShowNotificationIcon(
            profile.preferences.showNotificationIcon ?? true
          );
        } catch (error) {
          console.error('Failed to load notification icon preference:', error);
        }
      }
    };

    loadNotificationIconPreference();
  }, [user]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      {showNotificationIcon && <NotificationsButton />}
    </View>
  );
}
