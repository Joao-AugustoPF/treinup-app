...
export default function ProgressScreen() {
  ...
  const loadMetrics = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const groups = await ProgressService.getMetricsByDate(
        user,
        currentGym.id
      );
      ...
      if (groups.length > 0) {
        const latest = await ProgressService.getLatestMetrics(
          user,
          currentGym.id
        );
  ...
  const prepareLineChartData = () => {
    const weightData = metricGroups.map(group => {
    ...
    const bodyFatData = metricGroups.map(group => {
      const date = new Date(group.date);
      const formattedDate = date.toLocaleDateString('pt-BR', {
        month: 'short',
        year: '2-digit',
              ...
              <View style={styles.section}>
                ...
                <View style={styles.chartCard}>
                  ...
                  <LineChart
                    ...
                    dataSet={[
                      {
                        data: lineChartData.filter(item => item && item.dataSet === 2),
                        color: '#FF4444',
