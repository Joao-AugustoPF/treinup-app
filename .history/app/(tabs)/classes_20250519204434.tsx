...
import { checkIn, cancelCheckIn, getClasses, getClassTypes } from '@/app/actions/classes';
import { DATABASE_ID, CLASSES_COLLECTION_ID, CLASS_BOOKINGS_COLLECTION_ID } from '@/src/services/api';
...
export default function ClassesScreen() {
  ...
  useEffect(() => {
    if (params.initialData) {
      ...
      setClassTypes(initialData.types);
  ...
  const handleCheckIn = async (classId: string) => {
    Alert.alert('Confirmar Check-in', 'Deseja fazer check-in nesta aula?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const response = await checkIn(classId);
            if (response.error) {
  ...
  const handleCancelCheckIn = async (classId: string) => {
    Alert.alert('Cancelar Check-in', 'Deseja cancelar seu check-in?', [
      { text: 'Não', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const response = await cancelCheckIn(classId);
            if (response.error) {
              Alert.alert('Erro', response.error);
              return;
            }
