...
export default function ProgressScreen() {
  ...
  // Prepara os dados de peso para o gráfico
  ...
  // Prepara os dados de gordura para o gráfico
  const prepareBodyFatData = () => {
    return metricGroups.map(group => {
      const date = new Date(group.date);
      const formattedDate = date.toLocaleDateString('pt-BR', {
        month: 'short',
  ...
  const bodyFatData = prepareBodyFatData();
        ...
        <View style={styles.content}>
          {metricGroups.length === 0 ? (
            ...
            <>
              ...
              {/* Substituindo o LineChart por um AreaChart moderno */}
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Progresso do Peso e Gordura</Text>
                <View style={[styles.chartCard, styles.modernChartCard]}>
                  ...
                  {/* Gráfico de Peso - tipo área moderno */}
                  <AreaChart
                    data={weightData}
                    height={200}
                    ...
                    getPointerProps={(pointer) => ({
                      ...
                      pointerStripColor: 'rgba(0, 230, 195, 0.2)',
                      pointerStripWidth: 2,
                      pointerColor: '#00E6C3',
                      radius: 6,
                      ...
                      pointerLabelComponent: () => (
                        <View style={styles.tooltipContainer}>
                          <Text style={styles.tooltipTitle}>Peso</Text>
                          <Text style={styles.tooltipValue}>{pointer.value} kg</Text>
                          <Text style={styles.tooltipDate}>{pointer.date}</Text>
                        </View>
                      ),
                    })}
                  />
                  
                  {/* Gráfico de Gordura Corporal - sobreposto */}
                  <View style={styles.overlayChartContainer}>
                    <AreaChart
                      ...
                      getPointerProps={(pointer) => ({
                        pointerStripHeight: 160,
                        pointerStripColor: 'rgba(255, 68, 68, 0.2)',
                        pointerStripWidth: 2,
                        pointerColor: '#FF4444',
                        radius: 6,
                        pointerLabelWidth: 100,
                        pointerLabelHeight: 90,
                        pointerLabelComponent: () => (
                          <View style={[styles.tooltipContainer, styles.tooltipRed]}>
                            <Text style={styles.tooltipTitle}>Gordura</Text>
                            <Text style={styles.tooltipValue}>{pointer.value}%</Text>
                            <Text style={styles.tooltipDate}>{pointer.date}</Text>
                          </View>
                        ),
                      })}
...
const styles = StyleSheet.create({
  ...
  overlayChartContainer: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    paddingTop: 15, // match the padding of the parent
  },
  tooltipContainer: {
    backgroundColor: 'rgba(0, 230, 195, 0.9)',
    borderRadius: 10,
    padding: 10,
    width: 100,
    alignItems: 'center',
