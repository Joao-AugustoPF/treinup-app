...
export default function ProgressScreen() {
  ...
  const loadMetrics = async () => {
    try {
      ...
      // Usar o tenantId do gym atual ou o padrão se não tiver gym selecionada
      const tenantId = currentGym?.id || DEFAULT_TENANT_ID;
      
      const groups = await ProgressService.getMetricsByDate(user, tenantId);
      setMetricGroups(groups);
      
      if (groups.length > 0) {
        const latest = await ProgressService.getLatestMetrics(user, tenantId);
