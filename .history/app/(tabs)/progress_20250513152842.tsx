...
import { ProgressService, type Metric, type MetricType, type MetricGroup } from '@/src/services/progress';
import { Ruler, Scale, TrendingDown, TrendingUp, Droplet, Heart, Activity } from 'lucide-react-native';
...
const METRIC_CATEGORIES = {
  composicaoCorporal: ['weight', 'body_fat_pct', 'lean_mass_pct', 'muscle_mass', 'bmi'],
  medidas: ['chest_circ', 'waist_circ', 'hip_circ', 'arm_circ', 'thigh_circ', 'calf_circ'],
...
export default function ProgressScreen() {
  ...
  const [latestMetrics, setLatestMetrics] = useState<Record<MetricType, number | undefined>>({} as Record<MetricType, number | undefined>);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedCategory, setSelectedCategory] = useState<'composicaoCorporal' | 'medidas' | 'saude'>('composicaoCorporal');
  ...
  const loadMetrics = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const groups = await ProgressService.getMetricsByDate(user, currentGym.id);
      setMetricGroups(groups);
      
      if (groups.length > 0) {
        const latest = await ProgressService.getLatestMetrics(user, currentGym.id);
  ...
  const getChange = (
    ...
    const latest = metricGroups[metricGroups.length - 1].metrics[metricType] || 0;
    const previous = metricGroups[metricGroups.length - 2].metrics[metricType] || 0;
  ...
  const isIncreasePositive = (metricType: MetricType): boolean => {
    ...
    const negativeIncreaseMetrics: MetricType[] = [
      'body_fat_pct', 'waist_circ', 'hip_circ', 'bmi', 'visceral_fat',
      'rest_hr', 'bp_systolic', 'bp_diastolic', 'body_temp', 'metabolic_age'
    ];
  ...
  const prepareMainMetrics = () => {
    ...
    return metricTypes.map(({type, icon}) => ({
      ...
      value: latestMetrics[type] !== undefined 
  ...
  const prepareLineChartData = () => {
    return metricGroups.flatMap(group => {
      ...
      const weightPoint = group.metrics.weight !== undefined ? {
      ...
      const bodyFatPoint = group.metrics.body_fat_pct !== undefined ? {
      ...
      return [weightPoint, bodyFatPoint].filter(Boolean);
    });
  };
  ...
  const prepareBodyMeasurements = () => {
    ...
    const measurementLabels = ['Peito', 'Cintura', 'Quadril', 'Braço', 'Coxa', 'Panturrilha'];
    const measurementTypes: MetricType[] = [
      'chest_circ', 'waist_circ', 'hip_circ', 'arm_circ', 'thigh_circ', 'calf_circ'
    ];
    
    return {
      initialValues: measurementTypes.map(type => firstGroup.metrics[type] ?? 0),
      currentValues: measurementTypes.map(type => latestGroup.metrics[type] ?? 0),
  ...
  const preparePieData = () => {
    ...
    return [
      { value: leanMass, color: '#00E6C3', text: `${leanMass.toFixed(1)}%`, legend: 'Massa Magra' },
      { value: bodyFat, color: '#FF4444', text: `${bodyFat.toFixed(1)}%`, legend: 'Gordura Corporal' },
    ];
  };
  ...
  const getCategoryMetrics = (category: 'composicaoCorporal' | 'medidas' | 'saude') => {
    ...
    return metricTypes.map(type => ({
      ...
      isPositiveChange: Number(getChange(type as MetricType)) > 0 
          ...
          {metricGroups.length === 0 ? (
            ...
            <>
              ...
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Progresso do Peso e Gordura</Text>
                <View style={styles.chartCard}>
                  <View style={styles.legendContainer}>
                    <View style={styles.legendItem}>
                      <View style={[styles.legendColor, { backgroundColor: '#00E6C3' }]} />
                  ...
                  <LineChart
                    ...
                    dataSet={[
                      {
                        data: lineChartData.filter(item => item.dataSet === 2),
                        color: '#FF4444',
              ...
              <View style={styles.section}>
                ...
                <View style={styles.categoryTabsContainer}>
                  <TouchableOpacity
                    style={[
                      styles.categoryTab,
                      selectedCategory === 'composicaoCorporal' && styles.categoryTabActive
                    ]}
                    onPress={() => setSelectedCategory('composicaoCorporal')}
                  >
                    <Text style={[
                      styles.categoryTabText,
                      selectedCategory === 'composicaoCorporal' && styles.categoryTabTextActive
                    ]}>
                      Composição
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      styles.categoryTab,
                      selectedCategory === 'medidas' && styles.categoryTabActive
                    ]}
                    onPress={() => setSelectedCategory('medidas')}
                  >
                    <Text style={[
                      styles.categoryTabText,
                      selectedCategory === 'medidas' && styles.categoryTabTextActive
                    ]}>
                      Medidas
                    </Text>
                  </TouchableOpacity>
                  ...
                  >
                    <Text style={[
                      styles.categoryTabText,
                      selectedCategory === 'saude' && styles.categoryTabTextActive
                    ]}>
                      Saúde
                    </Text>
                  </TouchableOpacity>
                ...
                <View style={styles.chartCard}>
                  {getCategoryMetrics(selectedCategory).map((metric) => (
                    <View key={metric.type} style={styles.detailedMetricRow}>
                      <Text style={styles.detailedMetricLabel}>{metric.label}</Text>
              ...
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Volume de Treino Semanal</Text>
                <View style={styles.chartCard}>
                  <BarChart
                    data={barChartData}
                    barWidth={30}
                  ...
                  {bodyMeasurements && (
                    <View style={styles.measurementsContainer}>
                      <View style={styles.legendContainer}>
                        ...
                        <View style={styles.legendItem}>
                          <View style={[styles.legendColor, { backgroundColor: 'rgba(255, 68, 68, 0.5)' }]} />
                      ...
                      <View style={styles.measurementsTable}>
                        ...
                        {bodyMeasurements.labels.map((label, index) => {
                          const initialValue = bodyMeasurements.initialValues[index];
                          const currentValue = bodyMeasurements.currentValues[index];
                          ...
                          return (
                            <View key={label} style={styles.tableRow}>
                              ...
                              <Text style={styles.tableCellText}>{initialValue} cm</Text>
                              <Text style={styles.tableCellText}>{currentValue} cm</Text>
                              <Text 
                                style={[
                      ...
                      <View style={styles.measurementsBars}>
                        {bodyMeasurements.labels.map((label, index) => {
                          ...
                          const maxValue = Math.max(...bodyMeasurements.initialValues, ...bodyMeasurements.currentValues);
                          const initialPercent = (initialValue / maxValue) * 100;
                          const currentPercent = (currentValue / maxValue) * 100;
                          
                          return (
                            <View key={label} style={styles.measurementBarGroup}>
                              <Text style={styles.measurementBarLabel}>{label}</Text>
                      ...
                      centerLabelComponent={() => (
                        <View style={styles.pieChartCenter}>
                          <Text style={styles.pieChartCenterText}>
                            {latestMetrics.weight !== undefined ? `${latestMetrics.weight}kg` : ''}
                          </Text>
                          <Text style={styles.pieChartCenterSubtext}>Peso Atual</Text>
                        </View>
                      )}
                    />
                  </View>
                  <View style={styles.legendContainer}>
                    {pieChartData.map((item, index) => (
                      <View key={index} style={styles.legendItem}>
                        ...
                        <Text style={styles.legendText}>{item.legend}: {item.text}</Text>
                      </View>
                    ))}
                  </View>
                </View>
