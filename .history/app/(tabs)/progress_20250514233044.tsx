    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{t('bodyMeasurements')}</Text>

          <View style={styles.metricsContainer}>
            {metrics.map((metric, index) => (
              <View key={index} style={styles.metricCard}>
                <View style={styles.metricHeader}>
                  <metric.icon size={18} color="#00E6C3" />
                  ...
                  {parseFloat(metric.change) !== 0 && (
                    <>
                      {parseFloat(metric.change) > 0 ? (
                        <TrendingUp
                          size={14}
                          color={metric.inverted ? '#FF4444' : '#00E6C3'}
                        />
                      ) : (
                        <TrendingDown
                          size={14}
                          color={metric.inverted ? '#00E6C3' : '#FF4444'}
                        />
                      )}
                      <Text
                        style={[
                          ...
                          {
                            color:
                              parseFloat(metric.change) > 0 !== metric.inverted
                                ? '#00E6C3'
                                : '#FF4444',
                          },
                        ]}
        ...
        <View style={styles.content}>
          {metricGroups.length === 0 ? (
            ...
            <>
                      ...
                      dataSet={[
                        {
                          ...
                          startFillColor: 'rgba(0,230,195,0.2)',
                        ...
                        },
                        {
                          ...
                          startFillColor: 'rgba(255,68,68,0.2)',
                          endFillColor: 'transparent',
                          thickness: 3,
                        },
                      ]}
              ...
              {/* Weight and Body Fat Line Charts */}
              {metricGroups.length > 0 && (
                <>
                  <View style={styles.section}>
                    ...
                    <View style={styles.chartContainer}>
                      <LineChart
                        data={metricGroups
                          .filter(
                            (group) => group.metrics.weight !== undefined
                          )
                          .map((group, index) => {
                            const date = new Date(group.date);
                            return {
                              value: group.metrics.weight || 0,
                              label:
                                index === 0 || index === metricGroups.length - 1
                                  ? `${date.getDate()}/${date.getMonth() + 1}`
                                  : '',
                              dataPointText: `${group.metrics.weight}${METRIC_UNITS.weight}`,
                            };
                          })}
                        width={chartWidth}
                        height={180}
                        ...
                        dataPointsRadius={4}
                        startFillColor="rgba(0, 230, 195, 0.2)"
                        endFillColor="rgba(0, 230, 195, 0.0)"
                        startOpacity={0.9}
                        endOpacity={0.1}
                        yAxisThickness={0}
                        xAxisColor="rgba(255,255,255,0.2)"
                        yAxisTextStyle={{ color: '#666' }}
                        xAxisTextStyle={{ color: '#666' }}
                  ...
                  <View style={styles.section}>
                    <Text style={styles.sectionTitle}>{t('bodyFat')}</Text>
                    <View style={styles.chartContainer}>
                      <LineChart
                        data={metricGroups
                          .filter(
                            (group) => group.metrics.body_fat_pct !== undefined
                          )
                          .map((group, index) => {
                            const date = new Date(group.date);
                            return {
                              value: group.metrics.body_fat_pct || 0,
                              label:
                                index === 0 || index === metricGroups.length - 1
                                  ? `${date.getDate()}/${date.getMonth() + 1}`
                                  : '',
                              dataPointText: `${group.metrics.body_fat_pct}${METRIC_UNITS.body_fat_pct}`,
                            };
                          })}
                        width={chartWidth}
                        height={180}
                        ...
                        dataPointsRadius={4}
                        startFillColor="rgba(255, 68, 68, 0.2)"
                        endFillColor="rgba(255, 68, 68, 0.0)"
                        startOpacity={0.9}
                        endOpacity={0.1}
                        yAxisThickness={0}
                        xAxisColor="rgba(255,255,255,0.2)"
        ...
        <View style={styles.section}>
          <TouchableOpacity
            ...
            onPress={() => {
              /* Implementar adição de medida */
            }}
          >
            <Text style={styles.addMeasurementText}>{t('recordMeasurement')}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </View>
