          ...
          {schedules.length === 0 ? (
            ...
            <View>
              ...
              {schedules.filter(
                ...
                <>
                  <TouchableOpacity 
                    onPress={() => setExpandedSections(prev => ({...prev, completed: !prev.completed}))}
                    style={styles.sectionHeader}
                  >
                    <CalendarCheck size={20} color="#8A2BE2" />
                    <Text style={[styles.sectionTitle, { color: '#8A2BE2' }]}>Avaliações Realizadas</Text>
                    <View style={{flex: 1}} />
                    <Text style={[styles.accordionIcon, { color: '#8A2BE2' }]}>
                      {expandedSections.completed ? '▼' : '▶'}
                    </Text>
                  </TouchableOpacity>
                  
                  {expandedSections.completed && schedules
                    .filter(
                      (s) => 
                        (s.status === 'booked' || s.status === 'completed') && 
                        new Date(s.date) < new Date()
              ...
              {schedules.filter(s => s.status === 'cancelled').length > 0 && (
                <>
                  <TouchableOpacity 
                    onPress={() => setExpandedSections(prev => ({...prev, cancelled: !prev.cancelled}))}
                    style={styles.sectionHeader}
                  >
                    <CalendarX size={20} color="#FF4444" />
                    <Text style={[styles.sectionTitle, { color: '#FF4444' }]}>Avaliações Canceladas</Text>
                    <View style={{flex: 1}} />
                    <Text style={[styles.accordionIcon, { color: '#FF4444' }]}>
                      {expandedSections.cancelled ? '▼' : '▶'}
                    </Text>
                  </TouchableOpacity>
                  
                  {expandedSections.cancelled && schedules
